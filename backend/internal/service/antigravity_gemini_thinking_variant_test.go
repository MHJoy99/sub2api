...
func TestResolveGeminiThinkingVariant(t *testing.T) {
	...
	tests := []struct {
		...
		// gemini-3.5/3.6/3.7/3.8-flash 的变体会被 resolveModelMapping 自动补齐，所以降级测试用一个不在默认映射表里的自定义型号
		{"only -high exists: low request degrades to high", map[string]string{
			"gemini-custom-flash-high": "gemini-custom-flash-high",
		}, "gemini-custom-flash", budget("1000"), "gemini-custom-flash-high", true},
